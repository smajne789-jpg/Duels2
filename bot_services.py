        ...
        async with self.lock:
            ...
            self.conn.execute(
                ...
                INSERT INTO gift_checks(
                    token, amount, activations_total, activations_left, required_deposit, password, description, photo_file_id, created_by, created_at, is_active
                ) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
                """,
                (
                    token,
    ...
    async def claim_gift_check(self, token: str, user_id: int, password: str | None = None) -> tuple[sqlite3.Row, sqlite3.Row]:
        ...
        async with self.lock:
            ...
            if float(user["total_deposit"]) < float(gift_check["required_deposit"]):
                raise ValueError(
                    f"Р”Р»СЏ Р°РєС‚РёРІР°С†РёРё РЅСѓР¶РЅРѕ РїРѕРїРѕР»РЅРёС‚СЊ РјРёРЅРёРјСѓРј РЅР° {fmt_amount(float(gift_check['required_deposit']))} {self.config.bot_asset}"
                )
