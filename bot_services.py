...
class Database:
    ...
    async def create_withdrawal(self, check_id: str, user_id: int, amount: float, asset: str, check_url: str) -> sqlite3.Row:
        async with self.lock:
            ...
            self.conn.execute(
                """
                INSERT OR REPLACE INTO withdrawals(check_id, user_id, amount, asset, status, check_url, created_at)
                VALUES(?, ?, ?, ?, 'created', ?, ?)
                """,
                (str(check_id), user_id, round(amount, 8), asset, check_url, now_iso()),
            )
            self.conn.execute(
                """
                UPDATE users
    ...
    async def join_room(
        ...
        async with self.lock:
            ...
            self.conn.execute(
                """
                UPDATE users
                SET balance = ?, total_win = total_win + ?, total_wager = total_wager + ?
                WHERE user_id = ?
                """,
                (
            ...
            self.conn.execute(
                "UPDATE users SET total_wager = total_wager + ? WHERE user_id = ?",
                (amount, loser_id),
            )
            self.conn.execute(
    ...
    async def create_promocode(self, code: str, amount: float, activations: int, created_by: int) -> None:
        code = code.strip().upper()
        async with self.lock:
            self.conn.execute(
                """
                INSERT OR REPLACE INTO promocodes(code, amount, activations_left, created_by, is_active, created_at)
                VALUES(?, ?, ?, ?, 1, ?)
                """,
                (code, round(amount, 8), activations, created_by, now_iso()),
            )
            self.conn.commit()
    ...
    async def activate_promocode(self, code: str, user_id: int) -> tuple[sqlite3.Row, sqlite3.Row]:
        ...
        async with self.lock:
            ...
            self.conn.execute(
                "UPDATE users SET balance = ? WHERE user_id = ?",
                (round(float(user["balance"]) + float(promo["amount"]), 8), user_id),
            )
            self.conn.execute(
    ...
    async def get_stats(self) -> dict[str, Any]:
        async with self.lock:
            ...
            withdrawals = self.conn.execute("SELECT COALESCE(SUM(amount), 0) AS total FROM withdrawals WHERE status = 'created'").fetchone()["total"]
        return {
