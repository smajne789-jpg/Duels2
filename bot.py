...
async def throw_real_dice(bot: Bot, user_id: int, intro_text: str) -> int:
    await bot.send_message(user_id, intro_text)
...
async def cb_join_room(callback: CallbackQuery) -> None:
    ...
    creator_roll = await throw_real_dice(
        callback.bot,
        creator_id,
        f"🎲 Комната #{room_id}\nК вам зашел соперник. Бот кидает ваш реальный кубик Telegram.",
    )
    opponent_roll = await throw_real_dice(
        callback.bot,
        opponent_id,
        f"🎲 Комната #{room_id}\nСейчас бот кидает ваш реальный кубик Telegram.",
    )
    rerolls = 0
    while creator_roll == opponent_roll:
        ...
        await callback.bot.send_message(
            creator_id,
            f"🤝 В комнате #{room_id} выпала ничья {creator_roll}:{opponent_roll}. Перебрасываем реальные кубики Telegram.",
        )
        await callback.bot.send_message(
            opponent_id,
            f"🤝 В комнате #{room_id} выпала ничья {creator_roll}:{opponent_roll}. Перебрасываем реальные кубики Telegram.",
        ...
        creator_roll = await throw_real_dice(
            callback.bot,
            creator_id,
            f"🎲 Переброс для комнаты #{room_id}\nВаш новый реальный кубик Telegram:",
        )
        opponent_roll = await throw_real_dice(
            callback.bot,
            opponent_id,
