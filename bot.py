...
def admin_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="🎟 Создать чек", callback_data="admin:create_check")
    builder.button(text="🗂 Список чеков", callback_data="admin:checks")
    builder.button(text="💼 Комиссия проекта", callback_data="admin:house_commission")
    builder.button(text="📣 Рассылка", callback_data="admin:broadcast")
    builder.button(text="🏆 Топ рефов", callback_data="admin:refs_top")
...
def gift_check_admin_keyboard(token: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="🔗 Открыть чек", url=gift_check_link(token))
    builder.button(text="📣 Опубликовать в канал", callback_data=f"giftcheck:publish:{token}")
    builder.button(text="🗑 Удалить чек", callback_data=f"giftcheck:delete:{token}")
    builder.adjust(1)
    return builder.as_markup()
...
def gift_check_public_keyboard(token: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="🎟 Активировать", url=gift_check_link(token))
    builder.adjust(1)
    return builder.as_markup()
...
def check_skip_keyboard(step: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="⏭ Пропустить", callback_data=f"giftcheckskip:{step}")
    builder.adjust(1)
    return builder.as_markup()
...
def gift_check_manage_keyboard(token: str) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="🔗 Открыть чек", url=gift_check_link(token))
    builder.button(text="📣 Опубликовать", callback_data=f"giftcheck:publish:{token}")
    builder.button(text="🗑 Удалить", callback_data=f"giftcheck:delete:{token}")
    builder.button(text="🔙 К списку", callback_data="admin:checks")
    builder.adjust(1)
    return builder.as_markup()
...
def gift_check_list_keyboard(checks: list[Any]) -> InlineKeyboardMarkup:
    ...
    for gift_check in checks:
        status = "🟢" if int(gift_check["is_active"]) and int(gift_check["activations_left"]) > 0 else "⚫"
        builder.button(
            text=(
    ...
    builder.button(text="🔄 Обновить", callback_data="admin:checks")
...
def gift_check_subscription_keyboard(token: str, channels: list[str]) -> InlineKeyboardMarkup:
    ...
    for channel in channels:
        builder.button(text=f"📢 {channel}", url=f"https://t.me/{channel.lstrip('@')}")
    builder.button(text="✅ Проверить подписку", callback_data=f"giftchecksub:{token}")
    builder.adjust(1)
    return builder.as_markup()
...
def render_gift_check_text(gift_check: Any) -> str:
    ...
    return (
        f"🎁 <b>Чек на {fmt_amount(float(gift_check['amount']))} {config.bot_asset}</b>\n\n"
...
async def help_menu(message: Message) -> None:
    text = (
        ...
        "5. 🎟 Админ может создавать чеки и публиковать их в канал"
    )
