...
async def render_profile(user_id: int) -> str:
    ...
    wager_left = float(user["promo_wager_remaining"])
    return (
        "👤 <b>Ваш профиль</b>\n\n"
        f"💰 Баланс: <b>{fmt_amount(float(user['balance']))} {config.bot_asset}</b>\n"
        f"🎯 Ставка по умолчанию: <b>{fmt_amount(float(user['default_room_amount']))} {config.bot_asset}</b>\n"
        f"💸 Автовывод: <b>{'включен' if int(user['auto_withdraw_enabled']) else 'выключен'}</b>\n"
        f"🎁 Реф. доход: <b>{fmt_amount(float(user['referral_earnings']))} {config.bot_asset}</b>\n"
        f"👥 Рефералов: <b>{referral['referred_count']}</b>\n"
        f"🎮 Отыгрыш: <b>{fmt_amount(wager_left)} {config.bot_asset}</b>"
    )
...
async def show_main_menu(message: Message) -> None:
    ...
    text = (
        ...
        "💳 Пополнение и вывод работают через CryptoBot."
    )
...
async def show_game_menu(target: Message | CallbackQuery) -> None:
    text = (
        "🎲 <b>Игровое меню</b>\n\n"
        "🔥 Создавайте комнаты, заходите в чужие и играйте на реальные кубики Telegram.\n"
...
async def show_admin_panel(target: Message | CallbackQuery) -> None:
    ...
    text = (
        ...
        "⚙️ Управление промокодами, лимитами, админами, балансами, подпиской и логами."
    )
    if isinstance(target, CallbackQuery):
...
async def promo_menu(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        "Если у промокода есть вагер, вывод станет доступен только после полного отыгрыша."
    )
...
async def help_menu(message: Message) -> None:
    text = (
        ...
        "2. 🎲 Создайте комнату или зайдите в чужую\n"
        "3. 👀 У обоих игроков будут видны оба реальных кубика Telegram\n"
        "4. 🏆 Победитель забирает банк за вычетом комиссии\n"
        "5. 🎁 Если на промокоде есть вагер, вывод откроется только после отыгрыша"
    )
...
async def cb_join_room(callback: CallbackQuery) -> None:
    ...
    creator_roll = await send_duel_dice_to_both(
        callback.bot,
        creator_id,
        opponent_id,
        "1️⃣ Первый кубик — создатель комнаты",
        round_label,
    )
    opponent_roll = await send_duel_dice_to_both(
        callback.bot,
        opponent_id,
        creator_id,
        "2️⃣ Второй кубик — вошедший игрок",
        round_label,
    )
    ...
    while creator_roll == opponent_roll:
        ...
        creator_roll = await send_duel_dice_to_both(
            ...
            creator_id,
            opponent_id,
            "1️⃣ Переброс — кубик создателя комнаты",
            round_label,
        )
        opponent_roll = await send_duel_dice_to_both(
            callback.bot,
            opponent_id,
            creator_id,
            "2️⃣ Переброс — кубик вошедшего игрока",
            round_label,
        )
    ...
    duel_text = (
        f"🏆 <b>Дуэль завершена • Комната #{room_id}</b>\n\n"
    ...
    creator_result = "✅ <b>Ты выиграл!</b>\n\n" if winner_id == creator_id else "❌ <b>Ты проиграл</b>\n\n"
    opponent_result = "✅ <b>Ты выиграл!</b>\n\n" if winner_id == opponent_id else "❌ <b>Ты проиграл</b>\n\n"
...
async def cb_withdraw(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        f"🎮 Отыгрыш до вывода: <b>{fmt_amount(float(user['promo_wager_remaining']))} {config.bot_asset}</b>\n\n"
...
async def cb_admin_create_promo(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        "<code>NAME 1.5 100 10</code>\n\n"
        "Где:\n"
        "1. `NAME` — название\n"
        "2. `1.5` — сумма начисления\n"
        "3. `100` — активации\n"
        "4. `10` — вагер для вывода"
    )
    await callback.answer()
...
async def state_withdraw_amount(message: Message, state: FSMContext) -> None:
    ...
    if not user or float(user["balance"]) < amount:
        ...
        return
    if float(user["promo_wager_remaining"]) > 0:
        await message.answer(
            "🔒 <b>Вывод временно заблокирован</b>\n\n"
            f"Сначала нужно отыграть: <b>{fmt_amount(float(user['promo_wager_remaining']))} {config.bot_asset}</b>"
        )
        return
...
async def state_promo_code(message: Message, state: FSMContext) -> None:
    try:
        ...
        await message.answer(str(exc))
        return
    wager_requirement = float(promo["wager_requirement"])
    wager_text = (
        f"\n🎮 Отыгрыш до вывода: <b>{fmt_amount(wager_requirement)} {config.bot_asset}</b>"
        if wager_requirement > 0
        else "\n✅ Вывод доступен сразу после активации"
    )
    await message.answer(
        "🎁 <b>Промокод активирован</b>\n\n"
        f"💰 Начислено: <b>{fmt_amount(float(promo['amount']))} {config.bot_asset}</b>\n"
        f"💳 Баланс: <b>{fmt_amount(float(user['balance']))} {config.bot_asset}</b>{wager_text}"
    )
    await send_log(
        message.bot,
        "🎁 <b>Активация промокода</b>\n"
        ...
        f"Вагер: <b>{fmt_amount(float(promo['wager_requirement']))} {config.bot_asset}</b>",
    )
    await state.clear()
...
async def state_admin_promo(message: Message, state: FSMContext) -> None:
    ...
    if len(parts) not in {3, 4}:
        await message.answer("Формат: <code>NAME 1.5 100 10</code>")
        return
    code = parts[0].upper()
    try:
        amount = parse_amount(parts[1])
        activations = int(parts[2])
        wager_requirement = parse_amount(parts[3]) if len(parts) == 4 else 0.0
        if activations <= 0:
            raise ValueError
    except ValueError:
        await message.answer("Проверьте сумму, активации и вагер.")
        return
    await db.create_promocode(code, amount, activations, wager_requirement, message.from_user.id)
    await message.answer(
        f"🎁 Промокод <b>{code}</b> создан\n\n"
        f"💰 Сумма: <b>{fmt_amount(amount)} {config.bot_asset}</b>\n"
        f"♻️ Активаций: <b>{activations}</b>\n"
        f"🎮 Вагер: <b>{fmt_amount(wager_requirement)} {config.bot_asset}</b>"
    )
    await send_log(
        message.bot,
        "🎁 <b>Создан промокод</b>\n"
