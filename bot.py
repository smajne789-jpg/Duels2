...
async def finalize_gift_check_creation(message: Message, state: FSMContext, photo_file_id: str | None) -> None:
...
async def cb_gift_check_skip(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if step == "description":
        ...
        await finalize_gift_check_creation(callback.message, state, photo_file_id=None)
...
async def state_admin_check_image_photo(message: Message, state: FSMContext) -> None:
    if not await admin_guard(message):
        return
    await finalize_gift_check_creation(message, state, photo_file_id=message.photo[-1].file_id)
